...
def add_transaction():
    ...
    cursor.execute("""INSERT INTO transactions (user_id, type, category, amount, created_at) VALUES (%s, %s, %s, %s, NOW())""", (user_id, tx_type, category, amount))
    conn.commit()

    cursor.execute("""SELECT amount AS budget_amount FROM budgets WHERE user_id = %s AND category = %s""", (user_id, category))
    ...
    if budget_row:
        ...
        if total_spent + float(amount) > budget_amt:
...
def get_transactions(user_id, cursor):
    user_id = get_jwt_identity()
    conn = connect_db()
    if conn is None:
        return jsonify({"error": "Database connection failed"}), 500
    cursor = conn.cursor(dictionary=True)
    cursor.execute("""SELECT id, type, category, amount, created_at FROM transactions WHERE user_id = %s ORDER BY created_at DESC LIMIT 10""", (user_id,))
